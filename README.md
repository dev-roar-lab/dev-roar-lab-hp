...
このプロジェクトは[セマンティックバージョニング2.0.0](https://semver.org/lang/ja/)に従ってバージョン管理を行います。

**バージョン形式**: `MAJOR.MINOR.PATCH` (例: `1.0.0`)

| バージョン | 変更内容                                                   | 例                                      |
| ---------- | ---------------------------------------------------------- | --------------------------------------- |
| **MAJOR**  | 互換性のない破壊的変更（APIの変更、大規模なUI刷新など）    | `2.0.0`: UIフレームワークの完全刷新     |
| **MINOR**  | 後方互換性のある機能追加（新機能、コンポーネント追加など） | `1.1.0`: 新しいブログカテゴリ機能を追加 |
| **PATCH**  | 後方互換性のあるバグ修正（不具合修正、軽微な改善など）     | `1.0.1`: 日付表示のバグを修正           |
...
- UIコンポーネントの追加・変更・削除
- ページの追加・変更・削除
- ビルド成果物（`out/`, `docs-site/`）に影響する変更
- ユーザーに見える機能の追加・変更
- 依存関係の更新（Reactのメジャーアップデートなど）

❌ **バージョンアップ不要な変更**:

- CI/CDスクリプトの修正（`.github/workflows/`）
- 開発環境の設定変更（`.vscode/`, `.husky/`）
- ドキュメントのみの更新（`README.md`, `CLAUDE.md`）
- テストコードのみの変更・追加
- リンターやフォーマッター設定の変更
...
2. 該当する変更をコミット
3. Gitタグでバージョンをマーク: `git tag v1.0.0`
4. デプロイ時に自動的に全成果物にバージョンが反映されます
